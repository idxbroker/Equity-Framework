...
class Merlin_WXR_Parser {
	...
	function parse( $file ) {
		...
		if ( empty( $file ) ) {
			return new WP_Error( 'WXR_parse_error', esc_html__( 'Invalid WXR file.', 'stately' ) );
		}
		...
		if ( extension_loaded( 'simplexml' ) ) {
			...
			return new WP_Error( 'WXR_parse_error', esc_html__( 'No XML parser found.', 'stately' ) );
		}
	}
	...
	protected function simplexml_parse( $file ) {
		...
		if ( ! $xml ) {
			return new WP_Error( 'WXR_parse_error', esc_html__( 'Invalid WXR file.', 'stately' ), libxml_get_errors() );
		}
	...
	protected function xml_reader_parse( $file ) {
		...
		if ( ! $status ) {
			return new WP_Error( 'WXR_parse_error', esc_html__( 'Could not open the file for parsing', 'stately' ) );
		}
