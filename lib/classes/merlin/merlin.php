...
class Merlin {
	...
	protected function unset_default_widgets() {
		...
		$base_dir = get_stylesheet_directory() . '/' . $this->demo_directory;
	...
	public function svg( $args = array() ) {
		...
		if ( empty( $args ) ) {
			return __( 'Please define default parameters in the form of an array.', 'stately' );
		}

		// Define an icon.
		if ( false === array_key_exists( 'icon', $args ) ) {
			return __( 'Please define an SVG icon filename.', 'stately' );
		}
	...
	function steps() {
		...
		$this->steps = array(
			'welcome' => array(
				'name'    => esc_html__( 'Welcome', 'stately' ),
		...
		if ( class_exists( 'TGM_Plugin_Activation' ) ) {
			$this->steps['plugins'] = array(
				'name' => esc_html__( 'Plugins', 'stately' ),
		...
		if ( $this->get_base_content() ) {
			$this->steps['content'] = array(
				'name' => esc_html__( 'Content', 'stately' ),
				'view' => array( $this, 'content' ),
			);
		}

		$this->steps['ready'] = array(
			'name' => esc_html__( 'Ready', 'stately' ),
					...
					<li data-slug="<?php echo esc_attr( $slug ); ?>">
						...
						<span>
							...
							if ( isset( $plugins['install'][ $slug ] ) ) {
								$keys[] = esc_html__( 'Install', 'stately' );
							}
							if ( isset( $plugins['update'][ $slug ] ) ) {
								$keys[] = esc_html__( 'Update', 'stately' );
							}
							if ( isset( $plugins['activate'][ $slug ] ) ) {
								$keys[] = esc_html__( 'Activate', 'stately' );
							}
							echo implode( esc_html__( 'and', 'stately' ) , $keys );
							?>
	...
	function _ajax_plugins() {
		...
		foreach ( $plugins['activate'] as $slug => $plugin ) {
			if ( $_POST['slug'] === $slug ) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__( 'Activating', 'stately' ),
				);
				break;
			}
		}
		...
		foreach ( $plugins['update'] as $slug => $plugin ) {
			if ( $_POST['slug'] === $slug ) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__( 'Updating', 'stately' ),
				);
				break;
			}
		}
		...
		foreach ( $plugins['install'] as $slug => $plugin ) {
			if ( $_POST['slug'] === $slug ) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__( 'Installing', 'stately' ),
				);
				break;
			}
		}
		...
		if ( $json ) {
			...
			wp_send_json( array( 'done' => 1, 'message' => esc_html__( 'Success', 'stately' ) ) );
		}
	...
	function _ajax_content() {
		...
		if ( ! check_ajax_referer( 'merlin_nonce', 'wpnonce' ) || empty( $_POST['content'] ) && isset( $content[ $_POST['content'] ] ) ) {
			wp_send_json_error( array( 'error' => 1, 'message' => esc_html__( 'Invalid content!', 'stately' ) ) );
		}
		...
		if ( $json ) {
			...
			wp_send_json( array(
				'error'   => 1,
				'message' => esc_html__( 'Error', 'stately' ),
				'logs'    => '',
				'errors'  => '',
			) );
		}
	...
	protected function get_base_content() {
		...
		if ( ! empty( $content ) && is_array( $content ) ) {
			foreach ( $content as $slug => $data ) {
				...
				$content[ $slug ]['description'] = sprintf( esc_html__( 'Sample %s data.', 'stately' ), $slug );
				$content[ $slug ]['pending'] = esc_html__( 'Pending', 'stately' );
				$content[ $slug ]['installing'] = esc_html__( 'Installing', 'stately' );
				$content[ $slug ]['success'] = esc_html__( 'Success', 'stately' );
		...
		if ( file_exists( $base_dir . 'widgets.wie' ) ) {
			$content['widgets'] = array(
				'title'            	=> esc_html__( 'Widgets', 'stately' ),
				'description'      	=> esc_html__( 'Sample widgets data.', 'stately' ),
				'pending'          	=> esc_html__( 'Pending', 'stately' ),
				'installing'       	=> esc_html__( 'Installing', 'stately' ),
				'success'          	=> esc_html__( 'Success', 'stately' ),
		...
		if ( file_exists( $base_dir . 'slider.zip' ) ) {
			$content['sliders'] = array(
				'title'            	=> esc_html__( 'Sliders', 'stately' ),
				'description'     	=> esc_html__( 'Sample sliders data.', 'stately' ),
				'pending'         	=> esc_html__( 'Pending', 'stately' ),
				'installing'      	=> esc_html__( 'Installing', 'stately' ),
				'success'          	=> esc_html__( 'Success', 'stately' ),
		...
		if ( file_exists( $base_dir . 'customizer.dat' ) ) {
			$content['options'] = array(
				'title'            	=> esc_html__( 'Options', 'stately' ),
				'description'      	=> esc_html__( 'Sample theme options data.', 'stately' ),
				'pending'          	=> esc_html__( 'Pending', 'stately' ),
				'installing'       	=> esc_html__( 'Installing', 'stately' ),
				'success'          	=> esc_html__( 'Success', 'stately' ),
