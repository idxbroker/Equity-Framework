...
abstract class EQUITY_Admin {
	...
	public function create( $page_id = '', array $menu_ops = array(), array $page_ops = array(), $settings_field = '', array $default_settings = array() ) {
		...
		if ( isset( $this->menu_ops['submenu'] ) && ( isset( $this->menu_ops['main_menu'] ) || isset( $this->menu_ops['first_submenu'] ) ) )
			wp_die( sprintf( __( 'You cannot use %s to create two menus in the same subclass. Please use separate subclasses for each menu.', 'equity' ), 'EQUITY_Admin' ) );
	...
	public function maybe_add_main_menu() {
		...
		if ( isset( $this->menu_ops['main_menu']['sep'] ) ) {
			...
			if ( $sep['sep_position'] && $sep['sep_capability'] )
				$GLOBALS['menu'][$sep['sep_position']] = array( '', $sep['sep_capability'], 'separator', '', 'equity-separator wp-menu-separator' );
		}
	...
	public function register_settings() {
		...
		if ( ! equity_is_menu_page( $this->page_id ) )
			return;

		if ( equity_get_option( 'reset', $this->settings_field ) ) {
			if ( update_option( $this->settings_field, $this->default_settings ) )
				equity_admin_redirect( $this->page_id, array( 'reset' => 'true' ) );
			else
				equity_admin_redirect( $this->page_id, array( 'error' => 'true' ) );
			exit;
		}
	 ...
	 *
	 * Within Equity pages, the id attributes of form fields are the same as
	 ...
	 * @since 1.0
	 *
	 * @uses EQUITY_Admin:get_field_name() Construct name attributes for use in form fields.
	 *
	 * @param string $name Field name base
	 * @return string Full field name
	 */
	 ...
	 * @since 1.0
	 *
	 * @uses EQUITY_Admin::get_field_id() Constructs id attributes for use in form fields.
	 *
	 * @param string $id Field id base
	 * @return string Full field id
	 */
	 ...
	 *
	 * @uses EQUITY_Admin::get_field_value() Constructs value attributes for use in form fields.
	 *
	 * @since 1.0
	 *
	 * @param string $key Field key
...
/**
 * Abstract subclass of EQUITY_Admin which adds support for displaying a form.
 *
 ...
 * @package Equity\Admin
 */
abstract class EQUITY_Admin_Form extends EQUITY_Admin {
	...
	public function admin() {
		...
		<form method="post" action="options.php">
			...
			<p class="top-buttons">
				...
				submit_button( $this->page_ops['reset_button_text'], 'secondary equity-js-confirm-reset', $this->get_field_name( 'reset' ), false, array( 'id' => '' ) );
				?>
			</p>
...
/**
 * Abstract subclass of EQUITY_Admin which adds support for registering and
 ...
 * @package Equity\Admin
 */
abstract class EQUITY_Admin_Boxes extends EQUITY_Admin {
	...
	public function admin() {
		...
		?>
		<div class="wrap equity-metaboxes">
		<form method="post" action="options.php">
	...
	public function do_metaboxes() {
		...
		<div class="metabox-holder">
			<div class="postbox-container">
				<?php
				do_action( 'equity_admin_before_metaboxes', $this->pagehook );
				do_meta_boxes( $this->pagehook, 'main', null );
				if ( isset( $wp_meta_boxes[$this->pagehook]['column2'] ) )
					do_meta_boxes( $this->pagehook, 'column2', null );
				do_action( 'equity_admin_after_metaboxes', $this->pagehook );
				?>
			</div>
		</div>
		<?php
...
/**
 * Abstract subclass of EQUITY_Admin which adds support for creating a basic
 ...
 * @package Equity\Admin
 */
abstract class EQUITY_Admin_Basic extends EQUITY_Admin {

	/**
	 * Satisfies the abstract requirements of EQUITY_Admin.
	 *
