 ...
 * @link    
 */
class Equity_Featured_Listings_Scroller extends WP_Widget {

	function __construct() {
		$widget_ops  = array( 'classname' => 'equity-listing-scroller clearfix owlcarousel', 'description' => __( 'Display featured listings in a responsive scroller', 'equity' ), 'customize_selective_refresh' => true );
		$control_ops = array( 'width' => 300, 'height' => 350 );
		parent::__construct( 'equity-listing-scroller', __( 'Equity - Featured Listings Scroller', 'equity' ), $widget_ops, $control_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		$prev_link = apply_filters( 'listing_scroller_prev_link', $listing_scroller_prev_link_text = __( '<i class=\"fas fa-chevron-circle-left\"></i><span>Prev</span>', 'equity' ) );
		$next_link = apply_filters( 'listing_scroller_next_link', $listing_scroller_next_link_text = __( '<i class=\"fas fa-chevron-circle-right\"></i><span>Next</span>', 'equity' ) );
		...
		equityowljq(document).ready(function(){
			equityowljq(".equity-listing-scroller-' . $posts_per_scroller . '").owlCarousel({
				...
				responsive:{
					...
					450:{
						items: ' . round( $posts_per_scroller / 2 ) . '
					},
					800:{
						items: ' . $posts_per_scroller . '
					}
		...
		wp_add_inline_script( 'owl2', $script, 'after' );
		...
		echo $before_widget;
			...
			echo '<div class="listings-scroller equity-listing-scroller-' . $instance['posts_per_widget'] . ' owl-carousel owl-theme">';
			...
			if ( $wp_listings_widget_query->have_posts() ) : while ( $wp_listings_widget_query->have_posts() ) : $wp_listings_widget_query->the_post();
				...
				$loop = sprintf( '<div class="listing-widget-thumb"><a href="%s" class="listing-image-link"><img class="lazyOwl owl-lazy" data-src="%s" /></a>', get_permalink(), $thumbnail_src[0] );
				...
				$loop .= sprintf( '<a href="%s" class="button btn-primary more-link">%s</a>', get_permalink(), __( 'View Listing', 'equity' ) );
			...
			echo '</div><!-- .equity-listing-scroller -->';
			wp_reset_postdata();
	...
	function form( $instance ) {
		...
		printf(
			...
			__( 'How many results should be returned?', 'equity' ),
		...
		printf(
			'<p>%s <input type="text" name="%s" value="%s" size="3" /></p>',
			__( 'How many results should be shown in the widget at one time without scrolling?', 'equity' ),
		...
		<select id="'. $this->get_field_id( 'posts_term' ) .'" name="'. $this->get_field_name( 'posts_term' ) .'">
			<option style="padding-right:10px;" value="" '. selected( '', $instance['posts_term'], false ) .'>'. __( 'All Taxonomies and Terms', 'equity' ) .'</option>';
