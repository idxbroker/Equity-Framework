 ...
 */
function equity_get_image_id( $index = 0, $post_id = null ) {
 ...
 *
 * Applies `equity_get_image_default_args`, `equity_pre_get_image` and `equity_get_image` filters.
 *
 * @since 1.0
 *
 * @uses equity_get_image_id() Pull an attachment ID from a post, if one exists.
 *
 ...
 */
function equity_get_image( $args = array() ) {
	...
	/**
	 * A filter on the default parameters used by `equity_get_image()`.
	 *
	 * @since unknown
	 */
	$defaults = apply_filters( 'equity_get_image_default_args', $defaults, $args );

	$args = wp_parse_args( $args, $defaults );

	//* Allow child theme to short-circuit this function
	$pre = apply_filters( 'equity_pre_get_image', false, $args, get_post() );
	if ( false !== $pre )
		return $pre;
	...
	elseif ( 'first-attached' === $args['fallback'] ) {
		$id = equity_get_image_id( $args['num'], $args['post_id'] );
		$html = wp_get_attachment_image( $id, $args['size'], false, $args['attr'] );
		list( $url ) = wp_get_attachment_image_src( $id, $args['size'], false, $args['attr'] );
	}
	...
	return apply_filters( 'equity_get_image', $output, $args, $id, $html, $url, $src );
}
 ...
 * @since 1.0
 *
 * @uses equity_get_image() Return an image pulled from the media gallery.
 *
 * @param array|string $args Optional. Image query arguments. Default is empty array.
 *
 * @return false Returns false if URL is empty.
 */
function equity_image( $args = array() ) {

	$image = equity_get_image( $args );
 ...
 */
function equity_get_additional_image_sizes() {
 ...
 * @since 1.0
 *
 * @uses equity_get_additional_image_sizes() Return registered image sizes.
 *
 * @return array Two-dimensional, with width, height and crop sub-keys.
 */
function equity_get_image_sizes() {
	...
	$additional_sizes = equity_get_additional_image_sizes();
...
function equity_get_image_sizes_for_customizer() {

	$sizes = array();

	foreach ( (array) equity_get_image_sizes() as $name => $size ) {
