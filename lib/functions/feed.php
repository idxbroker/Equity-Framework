 ...
 *
 * @package Equity\Feeds
 * @author  IDX, LLC
 * @license GPL-2.0+
 * @link    
 */

add_filter( 'feed_link', 'equity_feed_links_filter', 10, 2 );
/**
 ...
 */
function equity_feed_links_filter( $output, $feed ) {

	$feed_uri = equity_get_option( 'feed_uri' );
	$comments_feed_uri = equity_get_option( 'comments_feed_uri' );
...
add_action( 'template_redirect', 'equity_feed_redirect' );
/**
...
function equity_feed_redirect() {
	...
	if ( $feed_uri && ! is_comment_feed() && equity_get_option( 'redirect_feed' ) ) {
		wp_redirect( $feed_uri, 302 );
		exit;
	}

	if ( $comments_feed_uri && is_comment_feed() && equity_get_option( 'redirect_comments_feed' ) ) {
